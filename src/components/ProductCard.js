...
export default function ProductCard({ product }) {
  ...
  // Second image for the hover-fade effect (first gallery image that differs).
  const secondImage = (images_gallery || []).find(
    (u) => u && u !== image_url
  );
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-cream overflow-hidden">
        ...
        />
        {/* second image fades in on hover (desktop) */}
        {secondImage && (
          <Image
            src={secondImage}
            ...
            sizes="(max-width:768px) 50vw, 25vw"
            loading="lazy"
            className="object-cover opacity-0 scale-105 transition duration-500 group-hover:opacity-100"
          />
        )}
