...
export default function ConfirmationClient({ id, orderNumber }) {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("sara_custom_confirmation");
  ...
  return (
    <div className="max-w-xl mx-auto px-5 py-14 text-center">
      ...
      <h1 className="text-2xl md:text-3xl font-black text-warm-mocha mb-2">
        تم استلام طلبك الخاص! ✨
      </h1>
      <p className="text-warm-mocha/70 font-bold leading-relaxed mb-6">
        شكرًا لثقتك في سارة دولز 🌸 سيراجع فريقنا طلبك ويتواصل معك قريبًا عبر
        واتساب لمناقشة التفاصيل وإرسال عرض السعر.
      </p>

      {/* reference number */}
      <div className="bg-cream rounded-3xl p-5 border border-pastel-pink/40 mb-6">
        <p className="text-sm font-bold text-warm-mocha/60 mb-1">رقم طلبك المرجعي</p>
